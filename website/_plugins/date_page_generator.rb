    ...
    def generate(site)
      ...
      archives.each_pair do |year, archive|
        ...
        archive.each_pair do |month, posts|
          ...
          site.pages << DatePage.new(site, site.source, File.join('rozhovory', year.to_s, month.to_s), "#{posts[0].date.strftime('%B')}, #{year}", posts)
        end

        site.pages << DatePage.new(site, site.source, File.join('rozhovory', year.to_s), year, year_posts)
      end

      site.pages << DateIndexPage.new(site, site.source, 'rozhovory', archives)
    end
  end
end
